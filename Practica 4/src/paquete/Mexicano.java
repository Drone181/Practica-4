...
public class Mexicano {
	...
	public void curp(String nac, String f, char g, boolean ex) {
		if(w==false) {
			...
			if(s1.equals("Defa")==true||s2.length()!=6) {
				...
				String s3=Valid.palAlti(Parte1curp.part1(Mayusculas.mayus(Nombre1), Mayusculas.mayus(Nombre2), Mayusculas.mayus(Apellido1), Mayusculas.mayus(Apellido2)));
				s3=s3.replace('Ñ','X');
	...
	public void rfc(String f) {
		if(x==false) {
			String s1 = Valid.palAlti(Parte1curp.part1(Mayusculas.mayus(Nombre1), Mayusculas.mayus(Nombre2), Mayusculas.mayus(Apellido1), Mayusculas.mayus(Apellido2)));
			s1=s1.replace('Ñ','X');
			String s2 = Fecha.fecha(f);
			if(s2.length()==6) {
